...
TKalmanFilter::TKalmanFilter(Point2f pt,float dt,float Accel_noise_mag)
{
	...
	kalman->statePre.at<float>(0) = pt.x; // x predicted state
	kalman->statePre.at<float>(1) = pt.y; // y 

	kalman->statePre.at<float>(2) = 0;
	kalman->statePre.at<float>(3) = 0;

	kalman->statePost.at<float>(0)=pt.x; //corrected state
	kalman->statePost.at<float>(1)=pt.y;

	setIdentity(kalman->measurementMatrix); // initializes scaled identity matrix //measurement matrix (H)

	kalman->processNoiseCov=(Mat_<float>(4, 4) << //process noise covariance matrix (Q)
	...
	setIdentity(kalman->measurementNoiseCov, Scalar::all(0.1)); //measurement noise covariance (R)

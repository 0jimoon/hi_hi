...
KuPathSmoothingPr::KuPathSmoothingPr()
{
...
/**
 ...
 @brief English: 
*/
list<KuPose> KuPathSmoothingPr::smoothingPath(list<KuPose> originalPathList)
{
	//Cubic과 Least를 한것이다.
...
list<KuPose> KuPathSmoothingPr::smoothingPathforprediction(list<KuPose> originalPathList)
{
	//Cubic과 Least를 한것이다.
	list<KuPose> WayPointList = generateWaypointFromPath(originalPathList, WAY_POINT_INTERVAL);//WAY_POINT_INTERVAL 간격마다 패스에서 경유점을 선택한다.
	list<KuPose> WayPointListAfterLeastSquare = doLeastSquare(WayPointList);//선정된 경유점들에서 Least를 적용한 새로운 경유점을 생성한다.
	list<KuPose> PathListAfterSmoothing = doCubicSplineInterpolation(0.1,WayPointListAfterLeastSquare);//0.1(m)간격으로 경유점들간의 패스를 선정한다.
	return PathListAfterSmoothing;
}
/**
...
@brief English: 
*/
list<KuPose> KuPathSmoothingPr::smoothingCubicSplinePath(list<KuPose> originalPathList)
{//Cubic만 한것이다.
...
/**
 ...
 @brief English: 
*/
list<KuPose> KuPathSmoothingPr::generateWaypointFromPath(list<KuPose> originalPathList,int ninterval)
{//패스로부터 경유점을 뽑는다.
...
@brief English: 
*/
list<KuPose> KuPathSmoothingPr::getWayPointList()
{
...
@brief English: 
*/
list<KuPose> KuPathSmoothingPr::getInflectionWayPointList()
{
...
@brief English: 
*/
list<KuPose> KuPathSmoothingPr::doLeastSquare(list<KuPose> WayPointList)
{	
	...
	for (; itThirdWayPoint != WayPointList.end(); itFirstWayPoint++,itSecondWayPoint++,itThirdWayPoint++)
	{
		...
		if(binitposflag){
			binitposflag=false;
			dWayPointX[0]=itFirstWayPoint->getX();	dWayPointY[0]=itFirstWayPoint->getY();
		}
		else{		
...
/**
 ...
 @brief English: 
*/
bool  KuPathSmoothingPr::Obstacleflag(int nmidPointX,int nmidPointY)
...
@brief English: 
*/
list<KuPose> KuPathSmoothingPr::doCubicSplineInterpolation(  double dinterval, list<KuPose> stlWayPoint)
{
...
/**
 ...
 @brief English: 
*/
list<KuPose> KuPathSmoothingPr::extractWayPoint(list<KuPose>PathList, int nPathCnt)
{
	int nCnt=0;

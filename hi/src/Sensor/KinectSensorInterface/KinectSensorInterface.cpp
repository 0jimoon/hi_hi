...
KinectSensorInterface::KinectSensorInterface()
{
	...
	m_nHightDataOfKinectRangeData =m_KuUtil.generateIntType1DArray(Sensor::KINECT_SENSOR_FOV,MAX_VAL);
	m_nTmpHightDataOfKinectRangeData=m_KuUtil.generateIntType1DArray(Sensor::KINECT_SENSOR_FOV,MAX_VAL);
...
KinectSensorInterface::~KinectSensorInterface()
{
	...
	cvReleaseImage(&m_IplColorImg);
	cvReleaseImage(&m_IplDepthColorImg);
	cvReleaseImage(&m_Ipl320ColorImg);
	cvReleaseImage(&m_Ipl320DepthColorImg);
...
/**
 ...
 @brief English: 
*/
int_1DArray KinectSensorInterface::getHightDataOfRangeData()
{
	return m_nHightDataOfKinectRangeData;
}
/**
		...
		if(fSelectedX[i] != -1 && fSelectedY[i] !=-1){
			...
			if(nDeg >=0 && nDeg < Sensor::KINECT_SENSOR_FOV){
				if(m_nTmpKinectRangeData[nDeg] > fSelectedX[i]){
					...
					m_nTmpHightDataOfKinectRangeData[nDeg] = fSelectedZ[i];
				}
...
void KinectSensorInterface::Kinect3DData()
{
	...
	for (XnUInt y = 0; y <MDYR; y+=2)
	{
		for (XnUInt x = 0; x < MDXR; x+=2 )
		{
			...
			m_fDistance[(int)(y/2)*320+(int)((int)320-x/2)]=pow(dTransZ*dTransZ+dTransY*dTransY+dTransX*dTransX , 1/3.0 )/100;
		}
	}
...
void KinectSensorInterface::execute()
{
		...
		for(int i=0; i<Sensor::KINECT_SENSOR_FOV; i++){
			...
			m_nHightDataOfKinectRangeData[i]=m_nTmpHightDataOfKinectRangeData[i] ;
			m_nTmpHightDataOfKinectRangeData[i]=MAX_VAL ;

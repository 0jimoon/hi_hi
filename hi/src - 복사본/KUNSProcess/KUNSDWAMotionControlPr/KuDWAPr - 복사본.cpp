...
{
  	m_dAlphaH =2; //heading //3
 	m_dAlphaC = 6; //clearance //5 
...
{ 
	...
	double dPredictedDiffRad = atan2((dGoalPosY - dPredicRobotPos[1] ),(dGoalPosX - dPredicRobotPos[0] )) - dPredicRobotPos[2];
...
// 					if(fabs(m_dObstacleY[k]) > m_dRadiusofRobot+2.0*m_nSafeDistance) continue; 
// 					else
						if(fabs(m_dObstacleY[k]) < (m_dRadiusofRobot+2.0*m_nSafeDistance)) nSafeDistance=m_nSafeDistance; 
					else nSafeDistance=m_nSafeDistance;
					double dCollisonDist = m_dObstacleX[k] -  sqrt(pow(m_dRadiusofRobot+nSafeDistance ,2) - pow(m_dObstacleY[k],2));
				...
				for(int k = 0; k<m_nIdX; k++){
					...
					if(fabs(m_dObstacleY[k]) < (m_dRadiusofRobot+m_nSafeDistance)) nSafeDistance=m_nSafeDistance; 
					else  nSafeDistance=m_nSafeDistance;					

					if( dCollisonDist > fabs(fabs(dIC)-(m_dRadiusofRobot-nSafeDistance)) && dCollisonDist < (fabs(dIC) +( m_dRadiusofRobot+nSafeDistance)) ){
						dThetaRad0 =M_PI/2 + dIC/fabs(dIC)*atan2( m_dObstacleY[k] - dIC, m_dObstacleX[k]);
						double temp=(dCollisonDist*dCollisonDist+ dIC*dIC - (m_dRadiusofRobot+nSafeDistance)*(m_dRadiusofRobot+nSafeDistance) )/(2.*fabs(dIC)*dCollisonDist);
...
KuDWAVelocity KuDWAPr::generateTRVelocity(KuPose TargetPos, KuPose RobotPos, int_1DArray  nSensorData )
{
	...
	if(fabs((double)m_nLeftWheelVel)<1&&fabs((double)m_nRightWheelVel)<1)
	{
		...
		if(dRVel>m_dRotateVelLimite)dRVel=m_dRotateVelLimite;
		else if(dRVel<-m_dRotateVelLimite) dRVel=-m_dRotateVelLimite;
		dRVel = dRVel*R2D;

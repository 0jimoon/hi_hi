...
void KuDWAPr::InitVar()
{
	...
	m_dRadiusofRobot = 320/10; //--> mm to cm //로봇 반경
	m_bFirstMove = true;
	m_dCosData=NULL;
	m_dSinData=NULL;
		...
		for(int j=0; j<DWA_SIZE; j++){
			...
			if(0==dRVelRad){ //병진 운동만 하는경우이다. 
				for(int k=0; k<m_nIdX ; k++){
					if(fabs(m_dObstacleY[k]) > m_dRadiusofRobot+m_nSafeDistance) continue; 
					else if(fabs(m_dObstacleY[k]) < (m_dRadiusofRobot+m_nSafeDistance)) nSafeDistance=m_nSafeDistance;
					else nSafeDistance=0;
				...
				for(int k = 0; k<m_nIdX; k++){
					...
					if(fabs(m_dObstacleY[k]) < (m_dRadiusofRobot+m_nSafeDistance)){
						double r = double(rand())/double(RAND_MAX);
						nSafeDistance=m_nSafeDistance*r; 
					}
					else  nSafeDistance=0;					

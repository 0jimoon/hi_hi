...
KuGradientPathPlannerPr::KuGradientPathPlannerPr()
{
	...
	m_dCellSize=100;
	m_math.setCellSizeMM(m_dCellSize);
...
@brief English:
*/
void KuGradientPathPlannerPr::setRadiusofRobot(int nRadiusofRobot)
{
...
@brief English:
*/
void KuGradientPathPlannerPr::setMap(int** nMap,double dCellSize)
{
	//m_smtnMap = nMap;
	m_math.setCellSizeMM(dCellSize);
	initIntCost(m_math.MM2AI(m_dRadiusofRobot)+2);
	...
	for(int i=0; i<m_nMapSizeX; i++){
		for(int j=0; j<m_nMapSizeY; j++){
			...
			else if(nMap[i][j]==KuMap::WARNING_AREA)
			{
...
{
	initialize(nMapSizeX, nMapSizeY);
}
/**
@brief Korean: 지도를 초기화하는 함수
@brief English:
*/
void KuGradientPathPlannerPr::initialize(int nMapSizeX, int nMapSizeY)
{
	...
	else if(m_nMap!=NULL&&(m_nMapSizeX!=nMapSizeX||m_nMapSizeY!=nMapSizeY))
	{
		for(int i = 0 ; i < m_nMapSizeX ; i++){
			delete[] m_nMap[i];
			delete[] m_fIntCost[i];
...
void KuGradientPathPlannerPr::initIntCost(int nRange )
{
	...
	m_nCSpaceInfinity=m_math.MM2AI(m_dRadiusofRobot);
	if(nRange<m_nCSpaceInfinity) nRange=m_nCSpaceInfinity;

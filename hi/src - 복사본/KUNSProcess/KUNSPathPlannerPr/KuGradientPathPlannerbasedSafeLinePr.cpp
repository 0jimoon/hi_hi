...
@brief English:
*/
KuGradientPathPlannerbasedSafeLinePr::KuGradientPathPlannerbasedSafeLinePr()
{
...
@brief English:
*/
void KuGradientPathPlannerbasedSafeLinePr::setRadiusofRobotp(int nRadiusofRobot)
{
...
@brief English:
*/
void KuGradientPathPlannerbasedSafeLinePr::setMap(int** nMap,double dCellSize)
{
		//m_smtnMap = nMap;
	  ...
	  if(m_dMap==NULL)
		...
		for(int i=0; i<m_nMapSizeX; i++){
			for(int j=0; j<m_nMapSizeY; j++){
				m_nMap[i][j] = nMap[i][j];
				m_dMap[i][j] = m_dProMapWeight;
				
				if(nMap[i][j]==KuMap::SAFE_AREA)
				{
					m_dMap[i][j] =1;
...
/**
@brief Korean: 지도 정보를 받아오는 함수m_dCellSize
@brief English:
*/
void KuGradientPathPlannerbasedSafeLinePr::setProMap(double** dMap)
{	
	if(m_dMap==NULL)
	{
		m_dMap = new double*[m_nMapSizeX];
	...
	for(int i=0; i<m_nMapSizeX; i++){
		for(int j=0; j<m_nMapSizeY; j++){
			if(dMap[(int)(i/10.0)][(int)(j/10.0)]>0.5)
			{
				m_dMap[i][j] =(int)(dMap[(int)(i/10.0)][(int)(j/10.0)]*m_dProMapWeight)+1;//KuMap::OCCUPIED_AREA;
			}
			else if(dMap[(int)(i/10.0)][(int)(j/10.0)]<0.5)
			{
				m_dMap[i][j] =1.0;
			}
		}
...
// 	sprintf_s(cMapPathName,"./data/map/testMap.txt");
// 
// 	saveProMap(cMapPathName,m_nMapSizeX,m_nMapSizeY, m_dMap);

}


void KuGradientPathPlannerbasedSafeLinePr::saveProMap(string strMapFilePath,int nMapSizeX,int nMapSizeY, double** dMap)
{
	int i = 0;
	int j = 0;
...
@brief English:
*/
void KuGradientPathPlannerbasedSafeLinePr::initializeMapSize(int nMapSizeX, int nMapSizeY)
{
...
@brief English:
*/
void KuGradientPathPlannerbasedSafeLinePr::initialize()
{
...
@brief English:
*/
void KuGradientPathPlannerbasedSafeLinePr::initMap()
{
	int** nMap = m_nMap;
...
@brief English:
*/
int KuGradientPathPlannerbasedSafeLinePr::generatePath(KuPose GoalPos, KuPose RobotPos)
{
	...
	//calCastPredictCost(m_nCSpaceObstacle);// set intrinsic cost
...
// 	for(int i=0; i<m_nMapSizeX; i++){
// 		for(int j=0; j<m_nMapSizeY; j++){
// 			m_dOutMap[i][j] = m_fRemovalCost[i][j];///100000.0;
// 		}
// 	}
// 
// 	KuDrawingInfo::getInstance()->setPathMap(m_dOutMap,1000,1000);
...
@brief English:
*/
void KuGradientPathPlannerbasedSafeLinePr::calNavCost()
{
    for (int i=0; i<m_nMapSizeX; i++) {
        for (int j=0; j<m_nMapSizeY; j++) {
...
@brief English:
*/
void KuGradientPathPlannerbasedSafeLinePr::calAdjCost(int nGoalPosition[2])
{
...
@brief English:
*/
bool KuGradientPathPlannerbasedSafeLinePr::calAdjCostUp(int nI, int nJ, float fValue)
{
        // boundary condition
		...
		if (m_nMap[nI-1][nJ] == KuMap::UNKNOWN_AREA) return false;		
		if (m_nMap[nI-1][nJ] == KuMap::OCCUPIED_AREA) return false;		
...
@brief English:
*/
bool KuGradientPathPlannerbasedSafeLinePr::calAdjCostDown(int nI, int nJ, float fValue)
{
        // boundary condition
        if (nI<1 || nI>m_nMapSizeX-2 || nJ<1 || nJ>m_nMapSizeY-2) return false;
        if (m_fIntCost[nI+1][nJ] == INFINITY_VALUE){m_fAdjCost[nI+1][nJ] = INFINITY_VALUE; return false;}	
...
@brief English:
*/
bool KuGradientPathPlannerbasedSafeLinePr::calAdjCostLeft(int nI, int nJ, float fValue)
{
        // boundary condition
        if (nI<1 || nI>m_nMapSizeX-2 || nJ<1 || nJ>m_nMapSizeY-2) return false;
        if (m_fIntCost[nI][nJ-1] == INFINITY_VALUE){m_fAdjCost[nI][nJ-1] = INFINITY_VALUE; return false;}	
		if (m_nMap[nI][nJ-1] == KuMap::UNKNOWN_AREA) return false;	
		if (m_nMap[nI][nJ-1] == KuMap::OCCUPIED_AREA) return false;		
...
@brief English:
*/
bool KuGradientPathPlannerbasedSafeLinePr::calAdjCostRight(int nI, int nJ, float fValue)
{
        // boundary condition
        if (nI<1 || nI>m_nMapSizeX-2 || nJ<1 || nJ>m_nMapSizeY-2) return false;
        if (m_fIntCost[nI][nJ+1] == INFINITY_VALUE) {m_fAdjCost[nI][nJ+1] = INFINITY_VALUE; return false;}	
...
@brief English:
*/
bool KuGradientPathPlannerbasedSafeLinePr::calAdjCostUpRight(int nI, int nJ, float fValue)
{
        // boundary condition
        if (nI<1 || nI>m_nMapSizeX-2 || nJ<1 || nJ>m_nMapSizeY-2) return false;
        if (m_fIntCost[nI-1][nJ+1] == INFINITY_VALUE){m_fAdjCost[nI-1][nJ+1] = INFINITY_VALUE; return false;}	
		if (m_nMap[nI-1][nJ+1] == KuMap::UNKNOWN_AREA) return false;	
		if (m_nMap[nI-1][nJ+1] == KuMap::OCCUPIED_AREA) return false;		
        // calculate adjcent cost of upper cell
...
@brief English:
*/
bool KuGradientPathPlannerbasedSafeLinePr::calAdjCostDownLeft(int nI, int nJ, float fValue)
{
        // boundary condition
        if (nI<1 || nI>m_nMapSizeX-2 || nJ<1 || nJ>m_nMapSizeY-2) return false;
        if (m_fIntCost[nI+1][nJ-1] == INFINITY_VALUE){m_fAdjCost[nI+1][nJ-1] = INFINITY_VALUE; return false;}	
...
@brief English:
*/
bool KuGradientPathPlannerbasedSafeLinePr::calAdjCostLeftUp(int nI, int nJ, float fValue)
{
        // boundary condition
        if (nI<1 || nI>m_nMapSizeX-2 || nJ<1 || nJ>m_nMapSizeY-2) return false;
        if (m_fIntCost[nI-1][nJ-1] == INFINITY_VALUE) {m_fAdjCost[nI-1][nJ-1] = INFINITY_VALUE; return false;}
...
@brief English:
*/
bool KuGradientPathPlannerbasedSafeLinePr::calAdjCostRightDown(int nI, int nJ, float fValue)
{
        // boundary condition
        if (nI<1 || nI>m_nMapSizeX-2 || nJ<1 || nJ>m_nMapSizeY-2) return false;
        if (m_fIntCost[nI+1][nJ+1] == INFINITY_VALUE){m_fAdjCost[nI+1][nJ+1]  = INFINITY_VALUE; return false;}	
...
@brief English:
*/
list<KuPose> KuGradientPathPlannerbasedSafeLinePr::getPath()
{
	return m_listPath;
}
/**
...
@brief English:
*/
void KuGradientPathPlannerbasedSafeLinePr::initIntCost(int nRange )
{
	...
	for(int i=0 ;i<2*nRange+1;i++) {	
		for(int j=0 ;j<2*nRange+1;j++) {	
			...
			if(ddistance<=6){//m_nCSpaceInfinity){
				m_finitPreCost[i][j] =CSPACE_PredictVALUE;
			}
			else if(ddistance>nRange-1)
			{
				m_finitPreCost[i][j] =0;
			}
			else{
				m_finitPreCost[i][j] =CSPACE_PredictVALUE*sqrt(1-pow(((ddistance-(double)m_nCSpaceInfinity)/(double)nRange),2));//타원 모델링	
			}
		}
	}
	//============================================================================
...
@brief English:
*/
void KuGradientPathPlannerbasedSafeLinePr::calCastIntrinCost(int nRange)
...
@brief English:
*/
void KuGradientPathPlannerbasedSafeLinePr::calCastPredictCost(int nRange)
{  
	for (int i=nRange ; i<m_nMapSizeX-nRange ; i++) {
		for (int j=nRange ; j<m_nMapSizeY-nRange ; j++){
			if ( m_dMap[i][j]>1){ //== KuMap::OCCUPIED_AREA ){ 
...
@brief English:
*/
bool KuGradientPathPlannerbasedSafeLinePr::extractGradientPath(int nStartPosition[2],int nGoalPosition[2])
{
	KuPose Path;
...
@brief English:
*/
int KuGradientPathPlannerbasedSafeLinePr::calGradient(int nX,int nY,int nGradientnum)
{
	double dCost[16]={0};
...
@brief English:
*/
void KuGradientPathPlannerbasedSafeLinePr::calAdjCostR(int nGoalPosition[2])
{
...
@brief English:
*/
bool KuGradientPathPlannerbasedSafeLinePr::calAdjCostUpR(int nI, int nJ, float fValue)
{
	// boundary condition
	if (nI<1 || nI>m_nMapSizeX-2 || nJ<1 || nJ>m_nMapSizeY-2) return false;
...
@brief English:
*/
bool KuGradientPathPlannerbasedSafeLinePr::calAdjCostDownR(int nI, int nJ, float fValue)
{
	// boundary condition
	if (nI<1 || nI>m_nMapSizeX-2 || nJ<1 || nJ>m_nMapSizeY-2) return false;
	if (m_fIntCost[nI+1][nJ] == INFINITY_VALUE){m_fRemovalCost[nI+1][nJ] = INFINITY_VALUE; return false;}	
...
@brief English:
*/
bool KuGradientPathPlannerbasedSafeLinePr::calAdjCostLeftR(int nI, int nJ, float fValue)
{
	// boundary condition
	if (nI<1 || nI>m_nMapSizeX-2 || nJ<1 || nJ>m_nMapSizeY-2) return false;
...
@brief English:
*/
bool KuGradientPathPlannerbasedSafeLinePr::calAdjCostRightR(int nI, int nJ, float fValue)
{
	// boundary condition
	if (nI<1 || nI>m_nMapSizeX-2 || nJ<1 || nJ>m_nMapSizeY-2) return false;
	if (m_fIntCost[nI][nJ+1] == INFINITY_VALUE) {m_fRemovalCost[nI][nJ+1] = INFINITY_VALUE; return false;}	
...
@brief English:
*/
bool KuGradientPathPlannerbasedSafeLinePr::calAdjCostUpRightR(int nI, int nJ, float fValue)
{
	// boundary condition
	if (nI<1 || nI>m_nMapSizeX-2 || nJ<1 || nJ>m_nMapSizeY-2) return false;
...
@brief English:
*/
bool KuGradientPathPlannerbasedSafeLinePr::calAdjCostDownLeftR(int nI, int nJ, float fValue)
{
	// boundary condition
	if (nI<1 || nI>m_nMapSizeX-2 || nJ<1 || nJ>m_nMapSizeY-2) return false;
	if (m_fIntCost[nI+1][nJ-1] == INFINITY_VALUE){m_fRemovalCost[nI+1][nJ-1] = INFINITY_VALUE; return false;}	
...
@brief English:
*/
bool KuGradientPathPlannerbasedSafeLinePr::calAdjCostLeftUpR(int nI, int nJ, float fValue)
{
	// boundary condition
	if (nI<1 || nI>m_nMapSizeX-2 || nJ<1 || nJ>m_nMapSizeY-2) return false;
	if (m_fIntCost[nI-1][nJ-1] == INFINITY_VALUE) {m_fRemovalCost[nI-1][nJ-1] = INFINITY_VALUE; return false;}
...
@brief English:
*/
bool KuGradientPathPlannerbasedSafeLinePr::calAdjCostRightDownR(int nI, int nJ, float fValue)
{
	// boundary condition
	if (nI<1 || nI>m_nMapSizeX-2 || nJ<1 || nJ>m_nMapSizeY-2) return false;
	if (m_fIntCost[nI+1][nJ+1] == INFINITY_VALUE){m_fRemovalCost[nI+1][nJ+1]  = INFINITY_VALUE; return false;}	
...
void KuGradientPathPlannerbasedSafeLinePr::setProMapWeight(double dProMapWeight)
{

...
void KuLocalPathPlannerPr::setData(KuPose RobotPos, vector<KuPose> WanderObstaclePose)
{
	...
	{
// 		double dm =RobotPos.getThetaRad();
...
void KuLocalPathPlannerPr::generatePredictionmap(KuPose RobotPos, KuPose*pKinectDataPose, IplImage* IplKinectCamera,vector<KuPose> vecWanderObstaclePose)
{
	...
	else
	{
		...
		{
			delete tracker.tracks[i];
		}
		tracker.tracks.clear();
	}

	/*
	if (m_prevecLocalObjPos.size()>0)
	{
		for (int i=0; i<m_vecLocalObjPos.size(); i++)
		{
			for (int j=i+1; j<m_vecLocalObjPos.size(); j++)
			{
				double dDist = m_math.calcDistBetweenPosesM(m_vecLocalObjPos[i], m_vecLocalObjPos[j]);
				double dAngle=fabs(m_vecLocalObjPos[i].getThetaRad()-m_vecLocalObjPos[j].getThetaRad());
				if(dDist < 1 && dAngle < 0.2)
				{
					if (m_prevecLocalObjPos[i].getObsID() == m_vecLocalObjPos[i].getObsID() && m_prevecLocalObjPos[i].getGroup() > 0)
					{
						m_vecLocalObjPos[i].setGroup(m_prevecLocalObjPos[i].getGroup());
						m_vecLocalObjPos[j].setGroup(m_prevecLocalObjPos[i].getGroup());
					}
					else
					{
						if (m_vecLocalObjPos[i].getGroup() >0)
						{
							m_vecLocalObjPos[j].setGroup(m_vecLocalObjPos[i].getGroup());
						}
						else
						{
							m_vecLocalObjPos[i].setGroup(a);
							m_vecLocalObjPos[j].setGroup(a);
							a++;
						}
					}
				}
...
// 		{
// 			for(int j=0; j<m_prevecObstaclePose.size();j++)
// 			{
// 				if(m_vecObstaclePose[i].getObsID()==m_prevecObstaclePose[j].getObsID())
// 				{					
// 					KuPose vecObst;
// 					KuPose vecRobot;
// 
// 					vecObst.setXm(m_vecObstaclePose[i].getXm()-m_prevecObstaclePose[j].getXm());
// 					vecObst.setYm(m_vecObstaclePose[i].getYm()-m_prevecObstaclePose[j].getYm());
// 
// 					vecRobot.setXm(m_preRobotPose.getXm()-RobotPos.getXm());
// 					vecRobot.setYm(m_preRobotPose.getYm()-RobotPos.getYm());
// 
// 					int nXstartPt=m_math.M2AI(m_vecObstaclePose[i].getXm());
// 					int nYstartPt=m_math.M2AI(m_vecObstaclePose[i].getYm());
// 
// 					int nXendPt= nXstartPt+m_math.M2AI((vecObst.getXm()+ vecRobot.getYm()));
// 					int nYendPt= nYstartPt+m_math.M2AI((vecObst.getYm()+ vecRobot.getYm()));
// 
// 					double dX= (vecObst.getXm()+vecRobot.getXm());
// 					double dY= (vecObst.getYm()+vecRobot.getYm());
// 
// 					if(checkstaticinMap(m_pMap->getMap(),nXstartPt,nYstartPt)&&fabs(vecObst.getXm())<1.0&&fabs(vecObst.getYm())<1.0)
// 					{
// 						for (int nx=-20; nx <21; nx++)
// 						{
// 							for (int mx=-20; mx <21; mx++)
// 							{
// 								if(checkEllipse( nXstartPt,  nYstartPt,dX*20.0, dY*20.0,nXstartPt+nx,nYstartPt+mx))	
// 								{
// 									m_pBuildingMap->getMap()[(int)(nXstartPt+nx)][(int)(nYstartPt+mx)]=KuMap::OCCUPIED_AREA;
// 								}
// 							}
// 						}
// 					}
...
void KuLocalPathPlannerPr::saveObstacledata(KuPose ObjPose)
{
	...
	else
	{
		...
		//ObjectTrajectory singleObjectTrajectory;
...
// 		for(int j=0; j<m_nMapSizeY;j++)
// 		{
// 			m_pCautionMap->getMap()[i][j]= m_pMap->getMap()[i][j];
// 		}
// 	}
// 	doCriticalSection(1);	
...
// 		vector<float> vecfdth;
		...
		for(int i=0; i<vecTunedTrajectory.size(); i++)
		{
			...
			if(vecTunedTrajectory[i].Tra_x.size()>4) //continue; 
			{
				...
				for (int j=1; j<dTime; j++)
				{
					RobotPos=calculatepredictRobotpos(RobotPos,dTVel,dRotDegVel,j);
	...
	//vecTunedTrajectory[i].PredictedPt[j].x
	//vecTunedTrajectory[i].PredictedPt[j].y

// 	if(m_bpredictexitObs==true)
// 	{
// 		CalculateGridPro(vecTunedTrajectory,m_mselObsID,m_dseltime,  0.7, 0.1);
// 		//CalculateGridPro(vecTunedTrajectory,1.0, 0.8);
// 	}
